...
class MALN(nn.Module):
    ...
    def __init__(self, args, dropout=0.1,fusion_dim=1024):
        ...
        self.specific_feature_extractor_text = nn.Sequential(
            nn.Linear(1024+100, 100),
            nn.Dropout(p=0.1),
            nn.Tanh())        
        self.specific_feature_extractor_visual = nn.Sequential(
            nn.Linear(512+100, 100),
            nn.Dropout(p=0.1),
            nn.Tanh())
        self.specific_feature_extractor_acoustic = nn.Sequential(
    ...
    def forward(self, training, groundTruth_labels, text, visual, acoustic, x1, x2, x3, x4, x5, x6, o1, o2, o3, qmask, umask, attention_mask=None, token_type_ids=None):
        ...
        # specific and common encoders
        
        specific_text = self.specific_feature_extractor_text(text)
        specific_visual = self.specific_feature_extractor_visual(visual)
        specific_audio = self.specific_feature_extractor_acoustic(acoustic)
        ...
        if training:
            ...
            specific_text_modal_pred = self.modal_discriminator(specific_text).view(-1, 3)
            specific_visual_modal_pred = self.modal_discriminator(specific_visual).view(-1, 3)
            specific_audio_modal_pred = self.modal_discriminator(specific_audio).view(-1, 3)
            ...
            # calculate the orthogonality loss, including the specific and common features
            
            specific_diff_loss = self.calculate_orthogonality_loss(specific_text, specific_visual) + self.calculate_orthogonality_loss(specific_text, specific_audio) + self.calculate_orthogonality_loss(specific_visual, specific_audio)
            common_diff_loss = self.calculate_orthogonality_loss(common_text, specific_text) + self.calculate_orthogonality_loss(common_visual, specific_visual) + self.calculate_orthogonality_loss(common_audio, specific_audio)
            
            # The type and ratio of the loss function can be adjusted according to the actual application scenario

            adv_specific_loss = self.adv_loss(specific_text_modal_pred, text_modal) + self.adv_loss(specific_visual_modal_pred, visual_modal) + self.adv_loss(specific_audio_modal_pred, audio_modal)
            ...
            all_loss = 5e-6 * (specific_diff_loss + common_diff_loss) + 0.01 * (adv_common_loss + adv_specific_loss)

            return  all_loss, L_AV_embedding, out, out_audio, out_text, out_visual, common_pred, DistanceCenter, specific_text, specific_visual, specific_audio, common_feature, common_text, common_visual, common_audio
        else:
            return L_AV_embedding, out, out_audio, out_text, out_visual, DistanceCenter, specific_text, specific_visual, specific_audio, common_feature, common_text, common_visual, common_audio
