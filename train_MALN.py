...
def train_or_eval_graph_model(model, loss_function, dataloader, epoch, cuda, optimizer=None, train=False):
    ...
    specific_text_original = torch.empty(0).type(torch.FloatTensor)
    specific_visual_original = torch.empty(0).type(torch.FloatTensor)
    specific_audio_original = torch.empty(0).type(torch.FloatTensor)
    ...
    if cuda:
        ...
        specific_text_original = specific_text_original.cuda()
        specific_visual_original = specific_visual_original.cuda()
        specific_audio_original = specific_audio_original.cuda()
    ...
    for data in dataloader:
        ...
        if train:
            ...
            all_loss, emotion_feature_0, log_prob, log_prob_audio, log_prob_text, log_prob_visual, common_pred, DistanceCenter, specific_text_0, specific_visual_0, specific_audio_0, common_feature_0, common_text_0, common_visual_0, common_audio_0 = model(train, label, roberta, visuf, acouf, x1, x2, x3, x4, x5, x6, o1, o2, o3, qmask, umask) 
            ...
            specific_text = specific_text_0.view(-1, specific_text_0.size()[-1])
            specific_audio = specific_audio_0.view(-1, specific_audio_0.size()[-1])
            specific_visual = specific_visual_0.view(-1, specific_visual_0.size()[-1])
        ...
        if not train:
            emotion_feature_0, log_prob, log_prob_audio, log_prob_text, log_prob_visual, DistanceCenter, specific_text_0, specific_visual_0, specific_audio_0, common_feature_0, common_text_0, common_visual_0, common_audio_0 = model(train, label, roberta, visuf, acouf, x1, x2, x3, x4, x5, x6, o1, o2, o3, qmask, umask)
            emotion_feature = emotion_feature_0.view(-1, emotion_feature_0.size()[-1])
            specific_text = specific_text_0.view(-1, specific_text_0.size()[-1])
            specific_audio = specific_audio_0.view(-1, specific_audio_0.size()[-1])
            specific_visual = specific_visual_0.view(-1, specific_visual_0.size()[-1])
        ...
        if train:
            ...
            specific_text_original = torch.cat([specific_text_original, specific_text], dim=0)
            specific_visual_original = torch.cat([specific_visual_original, specific_visual], dim=0)
            specific_audio_original = torch.cat([specific_audio_original, specific_audio], dim=0)
    ...
    specific_text_original = specific_text_original.data.cpu().numpy()
    specific_visual_original = specific_visual_original.data.cpu().numpy()
    specific_audio_original = specific_audio_original.data.cpu().numpy()
    ...
    return avg_loss, avg_accuracy, labels, preds, masks, avg_fscore, vids, ei, et, en, el, class_report, emotion_feature_original, specific_text_original, specific_visual_original, specific_audio_original, common_feature_original, common_text_original, common_visual_original, common_audio_original
...
if __name__ == '__main__':
    ...
    for e in range(n_epochs):
        ...
        if args.graph_model:
            ...
            test_loss, test_acc, test_label, test_pred, test_mask, test_fscore, _, _, _, _, _, _, emotion_feature_original, specific_text, specific_visual, specific_audio, common_feature, common_text, common_visual, common_audio = train_or_eval_graph_model(model, loss_function, test_loader, e, cuda)
        ...
        #  Save the features (including the specific, common features,labels, models and the global features) for visualization
            
        #     np.savetxt('./Features-tsne/best/feature_train.txt', emotion_feature_original_train)
        #     np.savetxt('./Features-tsne/best/specific_audio.txt', specific_audio)
        #     np.savetxt('./Features-tsne/best/common_feature.txt', common_feature)
        #     np.savetxt('./Features-tsne/best/specific_audio_train.txt', specific_audio_train)
